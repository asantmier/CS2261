...
# 1 "sprites.h" 1
# 23 "sprites.h"
extern const unsigned short congratSprite[2674];





extern const unsigned short foodSprite[208];





extern const unsigned short restartSprite[1548];





extern const unsigned short spikeSprite[529];





extern const unsigned short tama1Sprite[285];





extern const unsigned short tama2Sprite[468];





extern const unsigned short tama3Sprite[720];
# 4 "main.c" 2
...
Sprite food = { 0, 0, 0, 0, 0, 0, 16, 13, foodSprite };
...
Sprite spikes[] = {
     { 160 + 999, 0, 0, 0, 1, 0, 23, 23, spikeSprite },
     { 160 + 999, 0, 0, 0, 1, 0, 23, 23, spikeSprite },
     { 160 + 999, 0, 0, 0, 1, 0, 23, 23, spikeSprite }
    };
int numSpikes = 3;
int spikeTimer;
const int SPIKE_TIME = 1 * 60;



Sprite victory = { (160 / 2) - (14 / 2), (240 / 2) - (191 / 2), 0, 0, 0, 0, 14, 191, congratSprite };
Sprite restart = { 10, (240 / 2) - (172 / 2), 0, 0, 0, 0, 9, 172, restartSprite };
...
void update() {
 ...
 if (!foodActive && foodTimer > DROP_TIME) {
  food.row = 160 - food.height;

  int r = rand() % (240 - food.width - player.width);
 ...
 if (spikeTimer > SPIKE_TIME) {
  for (int i = 0; i < numSpikes; i++) {
   ...
   if (spikes[i].row > 160) {
    spikes[i].row = 0;
    spikes[i].col = rand() % (240 - spikes[i].width);
    spikeTimer = 0;
    break;
   }
  }
 ...
 if ((~(*(volatile unsigned short *)0x04000130) & ((1<<4)))) {
  int tmp = player.col + player.cDel;
  player.col = tmp + player.width > 240 ? (240 - 1) - (player.width - 1) : tmp;
 }
