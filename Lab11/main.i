...
# 1 "tiles.h" 1
# 21 "tiles.h"
extern const unsigned short tilesTiles[32];
...
# 1 "spritesheet.h" 1
# 21 "spritesheet.h"
extern const unsigned short spritesheetTiles[32];
...
void initialize() {
    ...
    DMANow(3, &tilesTiles, &((charblock *)0x6000000)[0], 64 / 2);
    DMANow(3, &tilesPal, ((unsigned short *)0x5000000), 512 / 2);

    DMANow(3, spritesheetTiles, &((charblock *)0x6000000)[4], 64 / 2);
...
void checkForLose() {
    ...
    if (pos.col < 0 || pos.col > 31 || pos.row < 0 || pos.row > 19 ||
            ((screenblock *)0x6000000)[31].tilemap[((pos.row) * (32) + (pos.col))] == 1) {

        u16 one = 1;
        DMANow(3, &one, &((screenblock *)0x6000000)[31], 1024 | (2 << 23));
        state = LOSE;
    }
}
...
void draw() {
    if (state == GAME) {
        u16 zero = 0;
        DMANow(3, &zero, &((screenblock *)0x6000000)[31], 1024 | (2 << 23));
...
# 175 "main.c"
            ...
            ((screenblock *)0x6000000)[31].tilemap[((curr->pos.row) * (32) + (curr->pos.col))] = 1;
            curr = curr->next;
        }
