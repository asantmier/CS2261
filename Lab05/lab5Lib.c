...
void drawRect4(int col, int row, int width, int height, volatile unsigned char colorIndex) {
    ...
    if (col & 1 && width & 1) { // col and width are both odd
        for(int r = 0; r < height; r++) {
            setPixel4(col, row + r, colorIndex);
            if (width / 2 != 0) {
                DMANow(3, &color, &videoBuffer[OFFSET(col + 1, row + r, SCREENWIDTH) / 2], DMA_SOURCE_FIXED | (width / 2));
            }
        }
    } else if (col & 1 && !(width & 1)) { // col is odd, width is even
        for(int r = 0; r < height; r++) {
            setPixel4(col, row + r, colorIndex);
            if ((width / 2) - 1 != 0) {
                DMANow(3, &color, &videoBuffer[OFFSET(col + 1, row + r, SCREENWIDTH) / 2], DMA_SOURCE_FIXED | ((width / 2) - 1));
            }
            setPixel4(col + width - 1, row + r, colorIndex);
        }
    } else if (!(col & 1) && width & 1) { // col is even, width is odd
        for(int r = 0; r < height; r++) {
            if (width / 2 != 0) {
                DMANow(3, &color, &videoBuffer[OFFSET(col, row + r, SCREENWIDTH) / 2], DMA_SOURCE_FIXED | (width / 2));
            }
            setPixel4(col + width - 1, row + r, colorIndex);
        }
    } else if (!(col & 1) && !(width & 1)) { // col and width are both even
        for(int r = 0; r < height; r++) {
            DMANow(3, &color, &videoBuffer[OFFSET(col, row + r, SCREENWIDTH) / 2], DMA_SOURCE_FIXED | (width / 2));
        }
    }
