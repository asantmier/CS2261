...
void drawInt3(int col, int row, int digits, int num, unsigned short color) {
 ...
 int charOffset = digits - 1;
    ...
    for (int i = 0; i < digits; i++) {
        drawChar3(col + (6 * (charOffset)), row, (char) ('0' + ((num % (10 * denominator)) / denominator)), color);
        denominator *= 10;
        charOffset--;
    }
}
...
void drawInt4(int col, int row, int digits, int num, unsigned char colorIndex) {
    ...
    int denominator = 1;
    for (int i = 0; i < digits; i++) {
        drawChar4(col + (6 * (charOffset)), row, (char) ('0' + ((num % (10 * denominator)) / denominator)), colorIndex);
        denominator *= 10;
        charOffset--;
    }
}
